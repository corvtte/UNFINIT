...
# ⚡ هاب چندپلتفرمه و موتور رسانه‌ای و فروشگاهی UNFINIT (v0.2.5)

[![Engine Version](https://img.shields.io/badge/version-v0.2.5-blue.svg)](https://github.com/corvtte/UNFINIT)
...
## 🌟 قابلیت‌های برجسته در نگارش v0.2.5

### ۱. شکار لینک ضدگلوله و بررسی چندمرحله‌ای CDN در تلگرام
- **رفع قفل وضعیت و پشتیبانی از کپشن:** حذف شرط مانع‌تراش وضعیت قبلی کاربر هنگام دریافت لینک دانلود مستقیم، ریست خودکار وضعیت معلق، و استخراج لینک از متن یا کپشن پیام‌های دارای پیش‌نمایش وب (`filters.text | filters.caption`).
- **بررسی تاب‌آور لینک (Fallback Range GET):** در صورتی که درخواست اولیه `HEAD` توسط سرور یا CDN فایل مسدود شود (403/405)، بلافاصله درخواست فالبک `GET` با هدر `Range: bytes=0-1024` و User-Agent مرورگر استاندارد ارسال می‌شود.
- **گزارش خطای شفاف:** در صورت عدم دسترسی یا نامعتبر بودن لینک، پیام خطای دقیق و راهنمای فارسی بدون هیچ‌گونه سکوت برای ادمین ارسال می‌گردد.

### ۲. تم‌بندی داینامیک و آنی اسکرول‌بار (Instant Themed Scrollbar)
- اتصال داینامیک استایل اسکرول‌بار به متغیر سراسری `--accent-color` در ریشه سند HTML.
- با تغییر تم در منوی رسمی آنتی‌گرویتی، اسکرول‌بارها بدون نیاز به رفرش صفحه فوراً به رنگ شاخص تم انتخابی (سایان، دراکولا، کتوپوچین، توکیو نایت و...) تغییر رنگ می‌دهند.

### ۳. پاکسازی کامل زرین‌پال و تجمیع متقارن سکرت‌ها
- حذف کامل فیلدها، متغیرها و المان‌های درگاه زرین‌پال از پنل وب و فرم‌ها به نفع درگاه شاپرکی بله و کارت‌به‌کارت.
- انتقال و تجمیع شناسه‌های ادمین‌ها و سوپرگروه تلگرام به تب اختصاصی سکرت‌ها و توکن‌ها (تب ۴).

### ۴. سیستم آپلود و تعویض لوگوی برند اختصاصی
- افزودن کارت آپلودر لوگو در تنظیمات سیستم با قابلیت پیش‌نمایش آنی، ذخیره‌سازی پایدار در دایرکتوری داده (`data/assets/logo.png`)، و کش‌باستینگ هوشمند بر اساس زمان در تمام بخش‌های پنل و گیت ورود.
